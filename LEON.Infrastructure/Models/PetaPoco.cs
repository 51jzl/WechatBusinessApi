...
#pragma warning disable 1066,1570,1573,1591
    ...
    public class Database : IDatabase
    {
        ...
        public Database()
        {
            ...
            providerName = !string.IsNullOrEmpty(entry.ProviderName) ? entry.ProviderName : "System.Data.SqlClient";
        ...
        public System.Collections.Generic.IEnumerable<T> FetchByPrimaryKeys<T>(System.Collections.Generic.IEnumerable<object> primaryKeys)
        {
            ...
            string arg = this._provider.EscapeSqlIdentifier(PocoData.ForType(typeof(T),_defaultMapper).TableInfo.PrimaryKey);
        ...
        protected void BuildPagingPrimaryKeyQueries<T>(long maxRecords, long skip, long take, string sql, ref object[] args, out string sqlCount, out string sqlPage)
        {
            ...
            {
                sql = AutoSelectHelper.AddSelectClause<T>(this._provider, sql, _defaultMapper);
            }
        ...
        protected void BuildPagingPrimaryKeyQueries(long maxRecords, long skip, long take, string primaryKey, string sql, ref object[] args, out string sqlCount, out string sqlPage)
        {
            ...
            sqlPage = this._provider.BuildPageQuery(skip, take, new SQLParts
            {
                Sql = sql,
                SqlCount = sqlCount,
        ...
        protected bool SplitSqlForPagingOptimized(long maxRecords, string sql, string primaryKey, out string sqlCount, out string sqlSelectRemoved, out string sqlOrderBy)
        {
            ...
            sqlCount = null;
            sqlOrderBy = null;
            Match match = new PagingHelper().RegexColumns.Match(sql);
            if (!match.Success)
            {
                return false;
            }
            ...
            if (new PagingHelper().RegexDistinct.IsMatch(sqlSelectRemoved))
            {
            ...
            {
                if (this.providerName.StartsWith("MySql"))
                {
            ...
            match = new PagingHelper().RegexOrderBy.Match(sqlCount);
            if (!match.Success)
            {
                sqlOrderBy = null;
            }
        ...
        /// <param name="primaryKeyName">The name of the primary key column of the table</param>
        /// <param name="autoIncrement">True if the primary key is automatically allocated by the DB</param>
        /// <param name="entity">The POCO objects that specifies the column values to be inserted</param>
        /// <param name="batchSize">The number of POCOS to be grouped together for each database rounddtrip</param>        
        public void BulkInsert(string tableName, string primaryKeyName, bool autoIncrement, IEnumerable<object> entity, int batchSize = 25)
        {
            try
            {
                OpenSharedConnection();
                try
                {
                    ...
                    lock (this.syncObj)
                    {
                        using (var cmd = CreateCommand(_sharedConnection, ""))
                        {
                            var pd = PocoData.ForObject(entity.First(), primaryKeyName,_defaultMapper);
                            ...
                            foreach (var i in pd.Columns)
                            {
                                ...
                                {
                                    // Setup auto increment expression
                                    string autoIncExpression = this._provider.GetAutoIncrementExpression(pd.TableInfo);
                                    if (autoIncExpression != null)
                                    {
                                        names.Add(i.Key);
                                    }
                                ...
                                names.Add(this._provider.EscapeSqlIdentifier(i.Key));
                            }
                            ...
                            do
                            {
                                ...
                                var index = 0;
                                foreach (var poco in entity.Skip(count).Take(batchSize))
                                {
                                    values.Clear();
                                    foreach (var i in pd.Columns)
                                    {
                                    ...
                                    if (autoIncrement)
                                    {
                                        outputClause = this._provider.GetInsertOutputClause(primaryKeyName);
                                    }

                                    cmd.CommandText += string.Format("INSERT INTO {0} ({1}){2} VALUES ({3})", this._provider.EscapeTableName(tableName),
        ...
        /// <param name="entity">The POCO objects that specifies the column values to be inserted</param>        
        /// <param name="batchSize">The number of POCOS to be grouped together for each database rounddtrip</param>        
        public void BulkInsert(IEnumerable<object> entity, int batchSize = 25)
        {
            if (!entity.Any()) return;
            var pd = PocoData.ForType(entity.First().GetType(),_defaultMapper);
            BulkInsert(pd.TableInfo.TableName, pd.TableInfo.PrimaryKey, pd.TableInfo.AutoIncrement, entity);
        }
        #endregion
